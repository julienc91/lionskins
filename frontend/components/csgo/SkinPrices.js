...
const SkinPrices = ({ skins, souvenir, statTrak }) => {
  ...
  if (!isWeapon) {
    ...
    qualities = Object.keys(Qualities).filter(
      (quality) => quality !== "vanilla"
    );
  }
    ...
    <Table unstackable celled singleLine textAlign="center">
      ...
      <Table.Body>
        {qualities.map((quality) => {
          ...
          const flatPrices = Object.keys(Providers)
            .filter((provider) => prices[provider])
            .map((provider) => prices[provider]);
