...
const Skin = ({ skin }) => {
  ...
  if (skin.type !== "weapons") {
    ...
    skinName +=
      skin.slug === "vanilla" ? t("csgo.qualities.vanilla") : skin.name;
  ...
  return (
    <Card color={getColorFromRarity(skin.rarity)} className="skin item">
      ...
      {skin.statTrak && (
        <Label className="stattrak" color="orange">
          {t("csgo.skin.stat_trak")}
        </Label>
      )}
      {skin.souvenir && (
        <Label className="souvenir" color="yellow">
      ...
      <Card.Content>
        ...
        <Card.Meta>{skin.quality ? t(Qualities[skin.quality]) : ""}</Card.Meta>
      </Card.Content>
      <Card.Content extra>
            ...
            .map((provider) => {
              ...
              return (
                <div className="price" key={provider}>
                  <MarketplaceLink
                    href={getSkinUrlFromProvider(skin, provider)}
                  >
