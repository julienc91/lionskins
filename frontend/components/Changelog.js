...
const Changelog = () => {
  ...
  const [lastDismissDate, setLastDismissDate] = useState(
    defaultLastDismissDate
  );

  const handleDismiss = () => {
    const newLastDismissDate = new Date();
    StorageManager.set(
      "changelog.dismiss_date",
      newLastDismissDate.toISOString()
    );
    ...
    <Message icon info onDismiss={handleDismiss}>
      ...
      <Message.Content>
        ...
        <ul>
          {messages.map((date, i) => (
            <li key={i}>
              {new Date(date).toLocaleDateString()} - {t(CHANGELOG[date])}
            </li>
          ))}
        </ul>
      </Message.Content>
